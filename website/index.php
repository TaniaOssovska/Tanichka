...
<div id="rs_allpage">
      ...
      <div style="display: flex; width: 100%; margin-top: 25px; text-align: left; -webkit-font-smoothing: antialiased;">
        <div style="background: #f9f9f9; border-radius: 5px; padding: 0px 30px 0px 30px; width: calc(100% - 60px);">
            <div style="font-family: 'Helvetica Neue', bold;  font-weight: bold; font-size: 36px; line-height: 42px; transform: uppercase; display: inline-block; width: 100%; z-index: 99999;">
              <p>Хіти продажів</p>
            </div>
        </div>
      </div>
  <?php 
  $select = mysqli_query($connect,"SELECT * FROM products WHERE hit_sales='1' AND availability='1' ORDER BY rating DESC");
